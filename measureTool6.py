...
class K6221():
 ...
 ############## configure source smu#####
    ...
    def ivt(self,ivtData): # smu1 is source and smu2 is sense
        ...
        time.sleep(tPoints*float(tInt) + 5)
        time.sleep(1)
        #self.sm.write(':SYST:COMM:SER:SEND ":TRAC:DATA?"')
        #time.sleep(5)
    ...
    def IVSweep(self,dcivData): # smu1 is source and smu2 is sense
        ...
        #### General measurement settings
        if loop: 
            sweepList = np.concatenate([np.linspace(start,stop,points),np.linspace(stop,start,points),np.linspace(start,-stop,points),np.linspace(-stop,start,points)])
            tPoints = 4*points
        else:
            tPoints = points
            sweepList = np.linspace(start,stop,points)
 ...
 ################ setting sweep points
        ...
        if pulse:
            self.sm.write('SOUR:LIST:CURR 0') ## clears the list and adds 0
            ...
            time.sleep(tPoints*float(pPeriod) + 5)
            self.sm.write('SOUR:SWE:ABOR')
            values=self.getTraceData('1','I',tPoints)
        else:
            self.sm.write('SOUR:LIST:CURR 0') ## clears the list and adds 0
            ...
            time.sleep(tPoints*float(stepPeriod) + 5)
            self.sm.write('SOUR:SWE:ABOR')
            self.turnOffOutputs()
            time.sleep(1)
            #self.sm.write(':SYST:COMM:SER:SEND ":TRAC:DATA?"')
...
######### Sense limit has an auto value
    def setSenseMode(self,channel,param,lim,nplc,aver):
        ...
        if lim.lower() == 'auto':
            ...
            self.sm.write(':SENS' + chan + ':'+mod+':PROT '+lim) #set compliance
...
############# IV sweep is a simple function to conduct IV sweeps on a single terminal that has to be passed via a dictionary
    def IVSweep(self,dcivData):
        ...
        self.sm.write(":sour"+schan+":"+mod+" "+str(start)) ##start value 
...
## with these, :trig:acq:del - (0.3 + 0.1)*period + 0.5*PW
        ...
        self.sm.write(":init (@"+schan+")") 
        time.sleep(tPoints*period + 3)## wait for sweep period and some 3 seconds
        res = self.sm.getTraceData(mSmu,mParam,tPoints)
...
######################## Script to run the actual measurement program ##########################
        ...
        if configData["mt"] == 'iv':
            ...
            while t1.is_alive():
                ...
                while not self.measureData.empty():
                    ...
                    PF.addPoint(plotData[0],plotData[1])
                PF.flushPlot()
            t1.join()
            f1.close()
    ...
    def runMOSFET(self,mosfetData):
        ...
        self.MD.setSenseMode(dSMU,dParam,dsLimit,nplc,aver)
        self.MD.setSenseMode(gSMU,gParam,gsLimit,nplc,aver)
        nPoints = len(drainList)*len(gateList)
        
        if ~sweepTerm: ##### drain is being swept
            drainData = {
                "source":dSMU,
                "sense":dSMU,
                "sParam":dsParam,
        ...
        ######################## prepare the didvData ###############
            ...
            for i in range(len(drainList)):
                ...
                dmeasure = self.MD.doMeasure(dSMU,dmParam,aver) # get gate current
                for j in range(gmeasure.points): 
                    if dParam == 'I':
                        self.measureData.put([gmeasure.V[j],gmeasure.I[j],dmeasure,drainList[j]]) # put all data
                    else:
                        self.measureData.put([gmeasure.V[j],gmeasure.I[j],drainList[j],dmeasure]) # put all data always as gV,gI,dV,dI
        self.MD.turnOffOutputs()
        self.active = False
...
class dIdVFrame(tk.Frame):
    def __init__(self,parent, configFrame, plotFrame):
        ...
        lbl_nAverage = tk.Label(master=self, text="Instrument Filter Average (NLPC)")
...
#### VG Settings###################
        ...
        rCount = rCount + 1
        lbl_dPoints = tk.Label(master=self, text="Sweep Terminal")
        lbl_dPoints.grid(row=rCount,column=0,sticky='e')
        c1=tk.Radiobutton(self,text="Drain",variable=self.sweepTerm,value=0)
        c1.grid(row=rCount,column=1)
        c1=tk.Radiobutton(self,text="Gate",variable=self.sweepTerm,value=1)
        ...
        r1 = tk.Radiobutton(self, text="False", variable=self.Loop, value=0)
        r1.grid(row=rCount,column=2)
        c1=tk.Checkbutton(self,text="Drain",variable=self.LoopDrain)
        c1.grid(row=rCount,column=3)
        c1=tk.Checkbutton(self,text="Gate",variable=self.LoopGate)
...
class controlFrame(tk.Frame):
    ...
    def switchFrame(self):
        ...
        if cfData["inst"] == 'B2912B':
            ...
            self.CF.MD = K6221()
        #### initiate logging ###########
        logging.basicConfig(filename= cfData["WD"] + '/' + cfData["mName"] + "-log.txt", filemode='a',format='%(asctime)s - %(levelname)s -%(message)s',level=logging.DEBUG)
#########################################################################################
